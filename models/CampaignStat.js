...
CampaignStatSchema.statics.searchWithAccessControl = async function (userId, query, options = {}) {
    ...
    // Filter stores based on granular permissions and get Klaviyo public IDs
    const accessibleKlaviyoIds = []
    for (const storeAccess of user.stores) {
        if (hasPermission(storeAccess, requiredPermission) && storeAccess.store_id) {
...
CampaignStatSchema.index({ klaviyo_public_id: 1 });
CampaignStatSchema.index({ store_public_ids: 1 });
CampaignStatSchema.index({ 'groupings.campaign_id': 1 });
CampaignStatSchema.index({ klaviyo_public_id: 1, 'groupings.campaign_id': 1 });
CampaignStatSchema.index({ store_public_ids: 1, send_time: -1 });
