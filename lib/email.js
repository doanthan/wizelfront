...
/**
 * Send invitation email to new user (no account yet)
 * @param {Object} options - Email options
 * @param {string} options.to - Recipient email
 * @param {string} options.contractName - Name of the contract they're being invited to
 * @param {string} options.roleName - Name of the role they're being assigned
 * @param {Array<string>} options.storeNames - Names of stores they'll have access to
 * @param {string} options.invitedBy - Name of the person who invited them
 * @param {string} options.token - Invitation token
 * @returns {Promise<Object>} - Resend response
 */
export async function sendNewUserInvitation({ to, contractName, roleName, storeNames, invitedBy, token }) {
  const invitationUrl = `${process.env.NEXTAUTH_URL}/accept-invite?token=${token}`;

  try {
    const resendClient = getResendClient();
    const data = await resendClient.emails.send({
      from: 'Wizel.ai <invitations@wizel.ai>',
      to: [to],
      ...
      html: `
        ...
        <html>
          <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>You've Been Invited - Wizel.ai</title>
          </head>
          <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f9fafb;">
            <table role="presentation" style="width: 100%; border-collapse: collapse;">
              <tr>
                  ...
                  <table role="presentation" style="max-width: 600px; width: 100%; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    ...
                    <tr>
                      <td style="padding: 40px;">
                        ...
                        <p style="margin: 0 0 20px; font-size: 16px; line-height: 24px; color: #1e293b;">
                          <strong>${invitedBy}</strong> has invited you to join <strong>${contractName}</strong> on Wizel.ai.
                        </p>

                        <!-- Invitation Details -->
                        <div style="margin: 30px 0; padding: 20px; background-color: #f8fafc; border-radius: 8px; border-left: 4px solid #8B5CF6;">
                          <p style="margin: 0 0 12px; font-size: 14px; line-height: 20px; color: #64748b;">
                            <strong style="color: #1e293b;">Your role:</strong> ${roleName}
                          </p>
                          ${storeNames && storeNames.length > 0 ? `
                            <p style="margin: 0; font-size: 14px; line-height: 20px; color: #64748b;">
                              <strong style="color: #1e293b;">Store access:</strong> ${storeNames.join(', ')}
                            </p>
                          ` : ''}
                        </div>

                        <p style="margin: 0 0 20px; font-size: 16px; line-height: 24px; color: #1e293b;">
                          To accept this invitation and create your Wizel.ai account, click the button below:
                        </p>
...
/**
 * Send notification email to existing user (already has account)
 * @param {Object} options - Email options
 * @param {string} options.to - Recipient email
 * @param {string} options.name - Recipient name
 * @param {string} options.contractName - Name of the contract they're being added to
 * @param {string} options.roleName - Name of the role they're being assigned
 * @param {Array<string>} options.storeNames - Names of stores they'll have access to
 * @param {string} options.invitedBy - Name of the person who added them
 * @returns {Promise<Object>} - Resend response
 */
export async function sendExistingUserNotification({ to, name, contractName, roleName, storeNames, invitedBy }) {
  try {
    const resendClient = getResendClient();
    const data = await resendClient.emails.send({
      from: 'Wizel.ai <notifications@wizel.ai>',
      to: [to],
      ...
      html: `
        ...
        <html>
          <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Added to Contract - Wizel.ai</title>
          </head>
          <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f9fafb;">
            <table role="presentation" style="width: 100%; border-collapse: collapse;">
              <tr>
                  ...
                  <table role="presentation" style="max-width: 600px; width: 100%; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    ...
                    <tr>
                      <td style="padding: 40px;">
                        ...
                        <p style="margin: 0 0 20px; font-size: 16px; line-height: 24px; color: #1e293b;">
                          <strong>${invitedBy}</strong> has added you to <strong>${contractName}</strong> on Wizel.ai.
                        </p>
                        ...
                        <div style="margin: 30px 0; padding: 20px; background-color: #f8fafc; border-radius: 8px; border-left: 4px solid #8B5CF6;">
                          ...
                          ` : `
                            <p style="margin: 0; font-size: 14px; line-height: 20px; color: #64748b;">
                              <strong style="color: #1e293b;">Store access:</strong> All stores in this contract
                            </p>
                          `}
                        </div>

                        <p style="margin: 0 0 20px; font-size: 16px; line-height: 24px; color: #1e293b;">
                          You can start working with this contract right away. Just log in to access it:
                        </p>
                        ...
                        <p style="margin: 30px 0 0; font-size: 13px; line-height: 18px; color: #94a3b8;">
                          If you have any questions about your new access, please contact ${invitedBy} or reach out to our support team.
                        </p>
                      </td>
                    </tr>
    ...
    console.log('✅ Existing user notification email sent successfully:', data);
    return { success: true, data };
  } catch (error) {
    console.error('❌ Error sending existing user notification email:', error);
    return { success: false, error: error.message };
  }
}
