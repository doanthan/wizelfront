      ...
      .map(async (store) => {
        try {
          ...
          if (statusFilter === 'scheduled') {
            // For dashboard - only scheduled campaigns
...
export async function fetchScheduledCampaigns(authOptions, channel = 'all') {
  try {
    ...
    // Query each channel
    const channelPromises = channels.map(async (channelObj) => {
      try {
        // Build filter: channel AND status = "Scheduled" only
        const filter = `equals(messages.channel,'${channelObj.filter}'),equals(status,"Scheduled")`;
      ...
      try {
        ...
        const included = data.included || []; // Get the included array with campaign-messages
          ...
          campaigns = campaigns.filter(campaign => {
            ...
            // For Scheduled/Sending campaigns, keep them all (even if in past, they might not have sent yet)
