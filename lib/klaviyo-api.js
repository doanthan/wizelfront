 ...
 * @param {Object} [options] - { apiKey, accessToken, clientId, clientSecret, refreshToken, payload, debug, ... }
 */
export async function klaviyoRequest(method, endpoint, options = {}) {
  ...
  // Debug logging - Initial request info (development only)
  const isDevelopment = process.env.NODE_ENV === 'development';
  const shouldDebug = debug && isDevelopment;

  if (shouldDebug) {
    console.log('🔵 Klaviyo API Request Debug:', {
      method,
      endpoint,
      url,
      authMethod: isOAuth ? 'OAuth' : 'API Key',
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (tokenManager) {
        ...
        if (shouldDebug) {
          console.log(`🔑 Using OAuth token (attempt ${attempt + 1}/${maxRetries + 1})`);
        }
      } else if (shouldDebug) {
        console.log(`🔑 Using API Key (attempt ${attempt + 1}/${maxRetries + 1})`);
      }

      // Build headers
      const headers = getKlaviyoHeaders(auth, isOAuth);

      // Debug: Show headers (without exposing full auth token)
      if (shouldDebug) {
      ...
      if (shouldDebug) {
        console.log(`📥 Response: ${res.status} ${res.statusText} (${responseTime}ms)`);
      }
      ...
      if (res.status === 401 && tokenManager) {
        ...
        if (shouldDebug) {
          console.log('⚠️ 401 Unauthorized - Token may be expired:', errorData);
        }
