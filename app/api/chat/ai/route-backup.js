 ...
 * Uses intelligent model routing: Haiku 4.5 → Sonnet 4.5 → Gemini 2.5 Pro fallback
 *
...
export async function POST(request) {
  try {
    ...
    // 4. Determine optimal model based on query complexity and context size
    const modelConfig = determineOptimalModel(sanitizedMessage, context);

    // 5. Build comprehensive system prompt (NEVER sent to user)
    const systemPrompt = secureSystemPrompt(buildSystemPrompt(context, session.user));

    // 6. Prepare conversation history (last 5 messages for context)
    ...
    // 7. Call AI with selected model (with automatic fallback)
    ...
    // 7.5. Clean up response - remove icon markers if using Haiku (it doesn't follow instructions well)
    let cleanedResponse = aiResponse.content;
    const isHaikuResponse = aiResponse.model && aiResponse.model.includes('haiku');

    if (isHaikuResponse) {
      // Remove all icon markers: [EMAIL], [GOAL], [QUICK], ICON_0, ICON_1, etc.
      ...
      console.log('🧹 Stripped icon markers from Haiku response');
    }
 ...
 * 1. Claude Haiku 4.5: Simple queries, on-screen data questions (20x cheaper, <500ms)
 * 2. Claude Sonnet 4.5: Complex analytical queries, strategic recommendations
 * 3. Gemini 2.5 Pro: Large context fallback, cost-effective for massive data
 */
function determineOptimalModel(query, context) {
  ...
  // Decision logic - PRIORITY ORDER: Haiku → Sonnet → Gemini

  // 1. Use Sonnet for complex analytical queries
  ...
  // 2. Use Sonnet for multi-part questions with substantial context
  ...
  // 3. Use Gemini for extremely large contexts (Haiku has limited context window)
  if (hasLargeContext) {
    return {
  ...
  // 4. DEFAULT: Use Haiku for simple queries (20x cheaper than Sonnet)
  return {
...
IMPORTANT:
- ONLY use the exact text markers shown above (e.g., [EMAIL], [GOAL], [QUICK])
- NEVER use numbered markers like ICON_0, ICON_1, ICON_2, etc.
- NEVER use placeholders like __ICON_N__
- Never explain what these markers mean - just use them directly
...
/**
 * Call AI with automatic fallback cascade
 *
 * Fallback strategy:
 * - Haiku 4.5 fails → Sonnet 4.5
 * - Sonnet 4.5 fails → Gemini 2.5 Pro
 * - Gemini is the final fallback (never a primary choice)
 */
async function callAIWithFallback({
  ...
  try {
    ...
    // If no fallback available (Gemini is the last resort)
    if (!fallbackModel) {
      console.error('❌ All AI models exhausted. No fallback available.');
    ...
    try {
      ...
      // If we were on Sonnet and it failed, try one more time with Gemini
      if (primaryModel.includes('sonnet') && fallbackModel.includes('gemini')) {
        throw new Error('Both primary and fallback AI models unavailable. Please try again later.');
      }
