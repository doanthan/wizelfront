        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{store?.name || 'Unknown Store'}</p>
            </div>
          </div>
        ...
        {!isScheduled && (
          <div className="grid grid-cols-3 gap-2 mt-3">
            <div className="text-center">
              ...
              <div className="text-xs font-medium">{(openRate * 100).toFixed(1)}%</div>
              <div className="text-xs text-gray-500">Open</div>
            </div>
            <div className="text-center">
              <MousePointer className="h-3 w-3 mx-auto mb-1 text-green-600" />
              <div className="text-xs font-medium">{(clickRate * 100).toFixed(1)}%</div>
              <div className="text-xs text-gray-500">Click</div>
            </div>
            <div className="text-center">
              <DollarSign className="h-3 w-3 mx-auto mb-1 text-purple-600" />
              <div className="text-xs font-medium">${revenue.toFixed(0)}</div>
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b dark:border-gray-700">
          ...
          <th className="text-left p-2 text-sm font-medium">Store</th>
          <th className="text-center p-2 text-sm font-medium">Time</th>
          <th className="text-center p-2 text-sm font-medium">Open Rate</th>
          <th className="text-center p-2 text-sm font-medium">Click Rate</th>
        ...
        {campaigns.map((campaign, idx) => {
          ...
          return (
            ...
            >
              ...
              <td className="p-2 text-sm">{store?.name || 'Unknown'}</td>
