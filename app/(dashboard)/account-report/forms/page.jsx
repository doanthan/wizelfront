...
export default function FormsReportRedirect() {
  ...
  useEffect(() => {
    if (!isLoadingStores && stores && stores.length > 0) {
      ...
      if (targetStore && targetStore.public_id) {
        // Redirect to the forms report for the first available store
        router.replace(`/store/${targetStore.public_id}/report/forms`);
      }
    }
