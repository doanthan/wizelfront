...
export default function CampaignsReportRedirect() {
  ...
  useEffect(() => {
    if (!isLoadingStores && stores && stores.length > 0) {
      ...
      if (targetStore && targetStore.public_id) {
        // Redirect to the campaigns report for the first available store
        router.replace(`/store/${targetStore.public_id}/report/campaigns`);
      }
    }
